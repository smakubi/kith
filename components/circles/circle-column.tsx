    ...
    <div className="flex flex-col min-h-[500px]">
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm text-slate-900">{circle}</h3>
          </div>
          <Button
            variant="ghost"
            size="sm"
            className="h-7 px-2 text-slate-400 hover:text-rose-500"
            onClick={onAddPerson}
          >
            <UserPlus className="h-3.5 w-3.5" strokeWidth={1.8} />
          </Button>
        </div>
        <p className="text-xs text-slate-400">{CIRCLE_DESCRIPTIONS[circle]}</p>
      </div>
      ...
      <div
        ...
        className={[
          ...
          isOver ? 'bg-rose-50 ring-2 ring-rose-200' : 'bg-slate-50',
        ].join(' ')}
      >
        {people.map((person) => (
        ...
        {people.length === 0 && (
          <div className="flex flex-col items-center justify-center h-32 text-center">
            <p className="text-xs text-slate-400">Drop contacts here</p>
          </div>
        )}
      </div>
    </div>
